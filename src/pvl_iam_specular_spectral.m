function [Rloss_Beam, Rloss_Iso, Rloss_Albedo] = pvl_iam_specular_spectral(SurfTilt, AOI, R_stack_allangles)
% Inputs:
...
assert(size(R_stack_allangles, 1) == 90, 'Expected 90 AOI rows in R_stack_allangles.');
assert(size(R_stack_allangles, 2) == 341, 'Expected 341 wavelength columns (300–2000nm).');
...
Rloss_Iso = R_stack_allangles' * iso_weights;  % 341×1
...
Rloss_Albedo = R_stack_allangles' * albedo_weights;  % 341×1
