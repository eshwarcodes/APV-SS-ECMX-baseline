...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[CcF,An,rs,Rdark,gsCO2,J,NPQ,Fvp,Fmp,E] = photosynthesis(Cc,IPAR,PPFD,Csl,ra,rb,Ts,Pre,RH,...
    CT,T0,Vmax,Oa,g1,go,rjv,theta,alpha)
%NPQ - Non-Photochemical Quenching
%Fvp - Variable fluorescence (photochemical)
%Fmp - Maximum fluorescence (non-photochemical)
...
if PPFD < 1e-3  % i.e., effectively zero light
    
    An = -Rdark;  % Only dark respiration remains
    J = 0;
    gsCO2 = go;  % Minimum conductance
    rsCO2 = 1 / gsCO2;
    CcF = Csl - An * Pre * (rsCO2 + rmes + 1.37 * rb + ra);
    rsH20 = (rsCO2/1.64)*(1e6);
    rs = rsH20*(Tf*Pre)/(0.0224*(Ts+273.15)*Pre0);
    CcF = max(CcF / (Pre * 1e-6), 0);  % Convert back to µmol/mol and clip
    NPQ = NaN; Fvp = NaN; Fmp = NaN;
    return;
end
...
switch ANSMOD
    case 1 % Fatichi
        ...
        J=min((-d2+sqrt(d2^2-4*d1*d3))/(2*d1),(-d2-sqrt(d2^2-4*d1*d3))/(2*d1)); % Electron transport rate 
...
%CiF(CiF<0) = 0;
CcF = Csl - An*Pre*(rsCO2 + rmes + 1.37*rb + ra); %%%%% Partial pressure of CO2 at chloroplast level [Pa]
CcF(CcF<0) = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
...
%% Transpiration
% Convert gsCO2 [micromol CO2/m2/s] to gsH2O [mol H2O/m2/s]
gsH2O = (gsCO2 / 1e6) * 1.6;  % Convert to mol/m2/s

% Compute vapor pressure deficit (VPD) [kPa]
es = 0.6108 * exp(17.27 * Ts / (Ts + 237.3));  % saturation vapor pressure [kPa]
ea = es * RH / 100;                            % actual vapor pressure [kPa]
VPD = max(es - ea, 0);                         % ensure non-negative

% Convert pressure to kPa
Pre_kPa = Pre / 1000;

% Compute transpiration [mol H2O/m2/s]
E = (VPD * gsH2O) / Pre_kPa;
