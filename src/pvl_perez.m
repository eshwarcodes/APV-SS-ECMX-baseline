function [SkyDiffuse,SkyDiffuse_Iso,SkyDiffuse_Cir,SkyDiffuse_Hor] = ...
    pvl_perez(SurfTilt, SurfAz, DHI, DNI, HExtra, SunZen, SunAz, AM,difHorznIrradNormHourlyMod,...
    dirHorznIrradNormHourlyMod,wavelengthDir,varargin)
...
DHI_spec = DHI*difHorznIrradNormHourlyMod;
DNI_spec = DNI*dirHorznIrradNormHourlyMod;

kappa = 1.041; %for SunZen in radians
z = SunZen*pi/180; % convert to radians

% 1. Spectral e_spec calculation, safely handling each time step
% Filter for valid DHI values
Dhfilter = DHI > 0;
...
SkyDiffuse_Iso = zeros(length(DHI),1);  % Preallocate as 24x1 vector of zeros
SkyDiffuse_Cir = zeros(length(DHI),1);  % Preallocate as 24x1 vector of zeros
SkyDiffuse_Hor = zeros(length(DHI),1);  % Preallocate as 24x1 vector of zeros
...
if any(idx)
    SkyDiffuse(idx) = 0;
    SkyDiffuse_Iso(idx) = 0;
    SkyDiffuse_Cir(idx) = 0;
    SkyDiffuse_Hor(idx) = 0;
    %warning('NaNs returned for SkyDiffuse < 0 or > max(HExtra).');
end
SkyDiffuse(SkyDiffuse<0)=0;
SkyDiffuse = SkyDiffuse(:);
SkyDiffuse_Iso(SkyDiffuse_Iso < 0) = 0;
SkyDiffuse_Iso = SkyDiffuse_Iso(:);
SkyDiffuse_Cir(SkyDiffuse_Cir < 0) = 0;
SkyDiffuse_Cir = SkyDiffuse_Cir(:);
SkyDiffuse_Hor(SkyDiffuse_Hor < 0) = 0;
